...
import type { DocumentRecord, SavedScope, UserRole } from "@/types/dashboard";
...
const selectScenario = (role: UserRole, query: string): Scenario => {
  ...
  if (role === "R&D") {
    if (normalized.includes("excursion") || normalized.includes("capa") || normalized.includes("root cause")) {
    ...
    if (normalized.includes("deviation") || normalized.includes("triage") || normalized.includes("capa")) {
      return msatDeviationScenario;
    }
    if (normalized.includes("audit") || normalized.includes("reviewer") || normalized.includes("pq")) {
...
export const DocumentIntelligenceShowcase = ({ query, role, documents = [], activeScope }: DocumentIntelligenceShowcaseProps) => {
  ...
  const contextualDocuments = selectedDocuments;
            ...
            {hasContextualInsight && primaryDocument ? (
              ...
              <div className="relative overflow-hidden rounded-[28px] border border-primary/25 bg-gradient-to-br from-primary/10 via-background to-background p-6 text-left shadow-[0_24px_60px_-40px_rgba(37,99,235,0.45)]">
                ...
                <p className="mt-3 text-xs text-muted-foreground/75">
                  The interactive charts and recommended actions stay active below while you line up prompts.
                </p>

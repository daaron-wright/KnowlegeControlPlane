...
export const CustomNode = memo(({ data, isConnectable, id }: NodeProps) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 p-4">
          ...
          {(data.showLabel || isExpanded) && data.description && (
            <div className="mt-2 text-xs font-medium leading-relaxed whitespace-pre-line" style={{ color: `${darkerColor}dd` }}>
              {data.description}
            </div>
          )}
      ...
      {/* Handles with improved styling - configurable positions */}
      {(() => {
        // Helper to convert string position to Position enum
        const getPosition = (pos: string): Position => {
          ...
          return Position.Left; // Default
        };

        // Helper to get positions array with string names - supports both string and array
        const getTargetPositions = (): Array<{ position: Position; name: string }> => {
          if (Array.isArray(data.targetPositions)) {
            return data.targetPositions.map((pos: string) => ({
              position: getPosition(pos),
              name: pos.toLowerCase(),
            }));
          }
          if (data.targetPosition) {
            return [{ position: getPosition(data.targetPosition), name: data.targetPosition.toLowerCase() }];
          }
          return [{ position: Position.Left, name: "left" }]; // Default
        };

        const getSourcePositions = (): Array<{ position: Position; name: string }> => {
          if (Array.isArray(data.sourcePositions)) {
            return data.sourcePositions.map((pos: string) => ({
              position: getPosition(pos),
              name: pos.toLowerCase(),
            }));
          }
          if (data.sourcePosition) {
            return [{ position: getPosition(data.sourcePosition), name: data.sourcePosition.toLowerCase() }];
          }
          return [{ position: Position.Right, name: "right" }]; // Default
        };

        const targetPositions = getTargetPositions();
        const sourcePositions = getSourcePositions();

        // Helper to get style based on position
        ...
        return (
          <>
            {targetPositions.map(({ position, name }, index) => (
              <Handle
            ...
            {sourcePositions.map(({ position, name }, index) => (
              <Handle
